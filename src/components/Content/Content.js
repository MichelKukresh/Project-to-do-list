...
export const Content = () => {
  ...
  const [cards, setCard] = useState([]); //Задания на сегодня
  const [cardsTomorrow, setCardsTomorrow] = useState([]);

  const handleClickGetTomorrow = (id, desc, title) => {

    const setCard = {id, body : desc, title};
    setCardsTomorrow([setCard, ...cardsTomorrow]);
    console.log(setCard);
    deleteItem(id);
  ...
  const getPosts = async () => {
    ...
    axios
      .get(
        "https://my-json-server.typicode.com/MichelKukresh/Project-to-do-list/posts"
      )
      .then((res) => {
        console.log(res);
        setCard(res.data);
        //setCardsTomorrow(res.data);
      })
      .catch((err) => {
        console.log("ERROR");
      });
  ...
  return (
    <div className={styles.wrapper}>
      <CardNew
        ...
        buttonClick={addCard}
      />
      <CardToday cards={cards} deleteItem={deleteItem} handleClickGetTomorrow={handleClickGetTomorrow} />
      <CardTomorrow cardsTomorrow={cardsTomorrow}/>
    </div>
  );
};
